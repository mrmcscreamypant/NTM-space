...
public class BiomeGenBaseTrash extends BiomeGenBase {
	
    public static final BiomeGenBase.Height height = new BiomeGenBase.Height(0.125F, 0.85F);

    //TODO: avoid doing an extra planets and make each planet unique and cool.
	public BiomeGenBaseTrash(int id) {
		super(id);
		this.setBiomeName("Trashy Plains");
		this.setDisableRain();
        ...
        this.topBlock = ModBlocks.block_scrap;
        this.fillerBlock = ModBlocks.block_scrap;
    ...
    public void genTerrainBlocks(World world, Random rand, Block[] blocks, byte[] meta, int x, int z, double noise)
    {
        ...
        for (int l1 = 255; l1 >= 0; --l1)
        {
            ...
            else
            {
                ...
                {
                    if (block2 == ModBlocks.block_scrap)
                    {
                        if (k == -1)
                        {
                            if (l <= 0)
                            {
                                block = null;
                                b0 = 0;
                                block1 = ModBlocks.block_electrical_scrap;
                            }
                            else if (l1 >= 59 && l1 <= 64)
                            {
                                block = this.topBlock;
                            ...
                            else if (l1 < 62)
                            {
                                ...
                                if (Math.random() > 0.4) {
                                	blocks[i2] = ModBlocks.block_electrical_scrap;
                                }
                                else
                                {
                                    blocks[i2] = ModBlocks.block_scrap;   	
                                }
                            }
                            else
                            {
